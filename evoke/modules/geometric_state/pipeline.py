...
from evoke.diffusers_version.pipeline_evoke_diffusers import (
    XLA_AVAILABLE,
    EvokePipelineOutput,
    EvokePipeline,
    calculate_shift,
...
@dataclass
class WarpAsHistoryPipelineOutput(EvokePipelineOutput):
    """geometric state output with optional warp debug tensors."""
...
def _default_geo_lora_path() -> str:
    return str((Path(__file__).resolve().parents[1] / GEO_DEFAULT_LORA_PATH).resolve())
...
class WarpAsHistoryPipeline(EvokePipeline):
    """Minimal geometric state inference pipeline.
    ...
    def _add_prompt_trigger(
        ...
        trigger: str | None = GEO_PROMPT_TRIGGER,
    ) -> str | list[str] | None:
    ...
    def _prepare_warp_state(
        ...
        if warp_invisible_fill not in GEO_INVISIBLE_FILL_MODES:
            raise ValueError(f"warp_invisible_fill must be one of {sorted(GEO_INVISIBLE_FILL_MODES)}.")

        device = self._execution_device
        window_num_frames = (GEO_NUM_LATENT_FRAMES_PER_CHUNK - 1) * self.vae_scale_factor_temporal + 1
        ...
        history_capacity = int(sum(GEO_HISTORY_SIZES))
        ...
        if any(size > cap for size, cap in zip(prev_chunk_history_sizes, GEO_HISTORY_SIZES)):
            raise ValueError(
                "prev_chunk_history_sizes cannot exceed the official GEO history slots "
                f"{GEO_HISTORY_SIZES!r}, got {prev_chunk_history_sizes!r}."
            )
    ...
    def _configure_geo_lora(self, lora_path: str | None) -> bool:
        if lora_path is None:
            self._set_geo_lora_enabled(False)
            return False

        adapter_path = self._materialize_geo_lora_path(lora_path, self._geo_adapter_name)
        loaded_path = getattr(self, "_geo_loaded_lora_path", None)
        if loaded_path != lora_path:
            if loaded_path is not None:
                self._delete_geo_adapter()
            self.load_lora_weights(adapter_path, adapter_name=self._geo_adapter_name)
    ...
    def _get_geo_runtime_lora_temp_dir(self) -> Path:
        temp_dir = getattr(self, "_geo_runtime_lora_temp_dir", None)
        if temp_dir is None:
            temp_dir = tempfile.TemporaryDirectory(prefix="geo_runtime_lora_")
            self._geo_runtime_lora_temp_dir = temp_dir
        return Path(temp_dir.name)

    def _materialize_geo_lora_path(self, lora_path: str | Path, adapter_name: str) -> str:
        ...
        if path.suffix == ".safetensors":
            ...
            if any(key.startswith("transformer.") for key in peft_state):
                return str(path)
            materialized = self._get_geo_runtime_lora_temp_dir() / f"materialized_prefixed_{safe_adapter_name}.safetensors"
            save_file(
        ...
        materialized = self._get_geo_runtime_lora_temp_dir() / f"materialized_{safe_adapter_name}.safetensors"
        save_file(
    ...
    def _geo_execution_device(self) -> torch.device:
    ...
    def _add_noise_to_warp_history_latents(
        ...
        noisy_chunks = []
        chunk_size = int(GEO_NUM_LATENT_FRAMES_PER_CHUNK)
    ...
    def _prepare_external_warp_latents(
        ...
        if warp_video_tensor is None:
            raise RuntimeError("geometric_state rollout is missing external warp video.")
        ...
        _, warp_latents_tensor = self.prepare_video_latents(
            ...
            num_latent_frames_per_chunk=GEO_NUM_LATENT_FRAMES_PER_CHUNK,
            dtype=torch.float32,
            device=device,
            generator=generator,
        )
    ...
    def _build_pyramid_base_histories(
        ...
        if chunk_index == 0:
            ...
            if source_frame is None:
                raise RuntimeError("geometric_state rollout is missing the previous chunk boundary frame.")
        ...
        if state.get("using_camera_warp", False):
            ...
            if warp_video_chunk is None or visibility_chunk is None:
                raise RuntimeError("geometric_state rollout is missing online warp conditioning.")
            ...
            if warp_video_tensor is None or visibility_mask is None:
                raise RuntimeError("geometric_state rollout is missing external warp state.")
            visibility_mask = visibility_mask.to(device=device, dtype=torch.float32)
            if frame_end > warp_video_tensor.shape[2]:
                raise RuntimeError(
                    "geometric_state rollout is missing warp frames for chunk "
        ...
        if base_latents_history_short is None or base_latents_history_short.shape[2] < 1:
            raise RuntimeError("geometric_state rollout is missing the official first-frame history prefix.")
        ...
        if state.get("using_camera_warp", False):
            ...
            latent_start = chunk_index * GEO_NUM_LATENT_FRAMES_PER_CHUNK
            latent_end = latent_start + GEO_NUM_LATENT_FRAMES_PER_CHUNK
            if latent_end > warp_latents_tensor.shape[2]:
                raise RuntimeError(
                    "geometric_state rollout is missing warp latent frames for chunk "
        ...
        visibility_latents = self._visibility_mask_to_history_latents(
            visibility_chunk,
            latent_frames=GEO_NUM_LATENT_FRAMES_PER_CHUNK,
        ...
        official_target_start = 1 + int(sum(GEO_HISTORY_SIZES))
        hidden_start = official_target_start
        if not state["rope_alignment"]:
            hidden_start += GEO_NUM_LATENT_FRAMES_PER_CHUNK
        ...
        warp_indices = torch.arange(
            ...
            official_target_start + GEO_NUM_LATENT_FRAMES_PER_CHUNK,
            device=device,
            dtype=torch.long,
        )
        ...
        if total_prev_history > 0:
            ...
            if chunk_index > 0:
                if prev_history_latent_window is None:
                    raise RuntimeError("geometric_state rollout is missing previous latent history.")
        ...
        if prefix_latent.shape[0] != warp_latents.shape[0] or prefix_latent.shape[-2:] != warp_latents.shape[-2:]:
            raise RuntimeError(
                "geometric_state first-frame prefix shape does not match warp history latents: "
    ...
    def init_autoregressive_state(
        ...
        *,
        ...
        height: int = 384,
        width: int = 640,
        num_frames: int = GEO_NUM_FRAMES,
        negative_prompt: str | list[str] | None = GEO_NEGATIVE_PROMPT,
        ...
        prev_chunk_history_sizes: list[int] | tuple[int, int, int] = GEO_PREV_CHUNK_HISTORY_SIZES,
        ...
        lora_active = self._configure_geo_lora(normalized_lora_path)
        ...
        if lora_prompt_embeds is not None and not lora_active:
            raise ValueError("lora_prompt_embeds is only supported when lora_path enables a GEO LoRA.")
        ...
        attention_kwargs = (
            {"history_visible_token_threshold": GEO_VISIBLE_TOKEN_THRESHOLD} if bool(visible_token_drop) else None
        )
        ...
        history_sizes = sorted(list(GEO_HISTORY_SIZES), reverse=True)
        ...
        indices = torch.arange(0, sum([1, *history_sizes, GEO_NUM_LATENT_FRAMES_PER_CHUNK]))
        (
            indices_prefix,
        ...
        ) = indices.split([1, *history_sizes, GEO_NUM_LATENT_FRAMES_PER_CHUNK], dim=0)
        ...
        state.update(
            {
                ...
                "pyramid_num_stages": GEO_PYRAMID_NUM_STAGES,
                "pyramid_num_inference_steps_list": list(GEO_PYRAMID_STEPS),
                "guidance_scale": 1.0,
                "use_zero_init": False,
                "zero_steps": 0,
    ...
    def _prepare_autoregressive_warp_chunk(
        ...
        device = self._geo_execution_device()
        height = int(state["height"])
        width = int(state["width"])
        window_num_frames = int(state["window_num_frames"])
    ...
    def _generate_next_chunk_from_state(self, state: dict[str, Any]) -> torch.Tensor:
        ...
        previous_geo_state = getattr(self, "_geo_state", None)
        ...
        try:
            ...
            self._geo_state = previous_geo_state
        ...
        current_latents = (
            real_history_latents[:, :, -GEO_NUM_LATENT_FRAMES_PER_CHUNK:].to(vae_dtype) / latents_std
            + latents_mean
        )
    ...
    def generate_next_chunk(
        ...
        *,
        ...
        """Generate one autoregressive GEO chunk and return the finalized video delta plus next state."""
    ...
    def finalize_autoregressive_state(
        ...
        *,
        ...
        self._current_timestep = None
        self._set_geo_lora_enabled(bool(state.get("lora_active", False)))
    ...
    def _run_original_evoke(
        ...
        *,
        ...
        evoke_kwargs: dict[str, Any],
    ) -> Any:
        if return_warp_debug or warp_debug_dir is not None:
            raise ValueError("Warp debug requires camera_poses or warp_video; original Evoke inference has no warp.")
        if warp_visibility_mask is not None:
            raise ValueError("warp_visibility_mask requires warp_video; omit both for original Evoke inference.")
        if target_intrinsics is not None:
            raise ValueError("target_intrinsics requires camera_poses; omit both for original Evoke inference.")
        if _normalize_optional_lora_path(lora_path) is not None:
            raise ValueError(
                "lora_path is only supported for geometric state conditioning. "
                "Pass camera_poses or warp_video to use a GEO LoRA, or omit lora_path for original Evoke inference."
            )
        ...
        **evoke_kwargs: Any,
        ...
        if warp_video is None and camera_poses is None:
            if lora_prompt_embeds is not None:
                raise ValueError("lora_prompt_embeds is only supported for geometric state conditioning.")
            return self._run_original_evoke(
                prompt=prompt,
                image=image,
                ...
                evoke_kwargs=evoke_kwargs,
            )
        if evoke_kwargs:
            unsupported = ", ".join(sorted(evoke_kwargs))
            raise ValueError(
                "Original Evoke arguments are only supported when neither warp_video nor camera_poses is provided: "
                f"{unsupported}."
            )
        if image is None:
            raise ValueError("image is required when using geometric state conditioning.")
    ...
    def stage2_sample(
        ...
        state = getattr(self, "_geo_state", None)
        if state is None:
        ...
        try:
            for i_s in range(pyramid_num_stages):
                use_geo_lora = bool(state["lora_active"]) and i_s == 0
                if use_geo_lora:
                ...
                for idx, t in enumerate(timesteps):
                    ...
                    # Stage2 synchronized warp: downsample the short-tier warp (and its visibility mask) to this
                    # stage's spatial resolution so warp tokens match the stage noise grid (train/infer parity).
                    # Inference is per-stage non-navit, so the legacy short-tier prepend then operates at stage res.
                    if (
                        attention_kwargs
                        and str(attention_kwargs.get("stage2_warp_compression_mode", "fixed_mem")) == "synchronized"
                        and current_latents_history_short is not None
                    ):
                        _ht, _wt = int(model_latents.shape[-2]), int(model_latents.shape[-1])
                        if (current_latents_history_short.shape[-2] != _ht
                                or current_latents_history_short.shape[-1] != _wt):
                            _lhs = current_latents_history_short
                            _b, _c, _tt, _h, _w = _lhs.shape
                            _lhs = _lhs.permute(0, 2, 1, 3, 4).reshape(_b * _tt, _c, _h, _w)
                            _lhs = F.interpolate(_lhs, size=(_ht, _wt), mode="bilinear")
                            current_latents_history_short = _lhs.reshape(_b, _tt, _c, _ht, _wt).permute(0, 2, 1, 3, 4)
                            if current_history_visible_mask_short is not None:
                                _m = current_history_visible_mask_short
                                _mb, _mc, _mt, _mh, _mw = _m.shape
                                _m = _m.permute(0, 2, 1, 3, 4).reshape(_mb * _mt, _mc, _mh, _mw)
                                _m = F.interpolate(_m, size=(_ht, _wt), mode="nearest")
                                current_history_visible_mask_short = _m.reshape(
                                    _mb, _mt, _mc, _ht, _wt
                                ).permute(0, 2, 1, 3, 4)
                    current_prompt_embeds = (
                        state.get("lora_prompt_embeds")
                        if use_geo_lora and state.get("lora_prompt_embeds") is not None
                        else prompt_embeds
                    )
            ...
            self._unfuse_geo_lora()
            self._set_geo_lora_enabled(bool(state["lora_active"]))
