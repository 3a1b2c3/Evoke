...
def read_cut_crop_and_resize(
    ...
    vr = PyVideoReader(video_path, threads=0)  # threads=0: let ffmpeg choose thread count
...
class BucketedSampler(Sampler):
    def __init__(
        ...
        # Sequence-parallelism distribution parameters
    ...
    def _shard_indices_for_sp_group(self, indices):
        """Return the subset of indices assigned to this SP group."""
        ...
        # Pad or truncate so length is divisible by num_sp_groups
        ...
        # Interleaved shard: this group takes every num_sp_groups-th element
    ...
    def __iter__(self):
        epoch_seed = self.seed + self._epoch  # epoch-specific seed for reproducibility
        ...
        for bucket_key, dataset_groups in self.dataset_buckets.items():
            ...
            # Shuffle before sharding for distributed consistency
            if self.shuffle:
...
if __name__ == "__main__":
    ...
    dataset_ratios = {}
    # dataset_ratios = {"<path/to/latents_a>": 0.9, "<path/to/latents_b>": 0.1}
