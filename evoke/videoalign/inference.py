...
class VideoVLMRewardInference:
    ...
    def _prepare_input(self, data):
        """Recursively move data to the target device."""
        if isinstance(data, Mapping):
            ...
            # TODO: add dtype handling if needed
...
def main():
    load_from_pretrained = "models/VideoReward"          # reward-model weights dir
    device = "cuda:0"
    ...
    with torch.no_grad():
        rewards_tensor = inferencer.reward(
            video_tensors,  # list of (T, C, H, W) tensors
            prompts,
            use_norm=True,
            return_batch_score=False,
        )
    ...
    # Verify consistency between path-input and tensor-input scores
    print(f"\n{'=' * 20} Verification {'=' * 20}")
