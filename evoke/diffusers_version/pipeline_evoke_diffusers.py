...
from diffusers.loaders import EvokeLoraLoaderMixin
from diffusers.models import AutoencoderKLWan, EvokeTransformer3DModel
from diffusers.pipelines.pipeline_utils import DiffusionPipeline
from diffusers.schedulers import EvokeScheduler
from diffusers.utils import is_ftfy_available, is_torch_xla_available, logging, replace_example_docstring
from diffusers.utils.torch_utils import randn_tensor
from diffusers.video_processor import VideoProcessor

from ..pipelines.pipeline_output import EvokePipelineOutput
...
def optimized_scale(positive_flat, negative_flat):
    ...
    # Optimal scale: projection of cond onto uncond direction.
...
class EvokePipeline(DiffusionPipeline, EvokeLoraLoaderMixin):
    r"""
    ...
    def prepare_image_latents(
        ...
        # Encode the i2v reference image deterministically (.mode() = mu) to match training, which
        #   uses .mode() for both the x0 anchor and the history 1x slot. Sampling here would give
        #   frame 0 -- the anchor of the whole rollout -- a jitter the student never saw. v2v keeps
        #.sample(), matching how v2v training encodes it.
        device = device or self._execution_device
        if latents is None:
            image = image.unsqueeze(2).to(device=device, dtype=self.vae.dtype)
            _dist = self.vae.encode(image).latent_dist
            _dist.sample(generator=generator)          # keep the RNG stream identical; draw discarded
            latents = _dist.mode()
        ...
        if fake_latents is None:
            ...
            _dist_f = self.vae.encode(fake_video).latent_dist
            _dist_f.sample(generator=generator)        # same: keep the RNG stream identical
            fake_latents_full = _dist_f.mode()
    ...
    def _decode_chunk_persistent_cache(
        ...
        """Decode one latent chunk reusing the VAE feat-map cache across chunks (no boundary artifacts)."""
        if is_first_chunk:
    ...
    def sample_block_noise(
        ...
        # Generator is required for reproducibility; fallback creates a non-deterministic one.
        ...
        cov = cov.float()  # fp32 required for stable Cholesky decomposition.
    ...
    def __call__(
        ...
        is_amplify_first_chunk: bool = False,
        # ------------ VAE decode ------------
        # "default": decode each chunk independently (low VRAM, boundary artifacts).
        # "long": decode full sequence at once (no artifacts, high VRAM peak).
        # "streaming": decode chunk with overlap warm-up frames then discard them.
        # "persistent": reuse vae._feat_map across chunks; bit-identical to long, low VRAM.
        vae_decode_type: str = "default",
        streaming_overlap_latent: int = 2,
        on_chunk_decoded=None,  # Callable[[int, torch.Tensor], None] called after each chunk is decoded.
    ):
        r"""
        ...
        if vae_decode_type in ("long", "streaming", "persistent"):
            num_target_latent_frames = (num_frames - 1) // self.vae_scale_factor_temporal + 1
            num_latent_chunk = max(
                1,
                (num_target_latent_frames + num_latent_frames_per_chunk - 1) // num_latent_frames_per_chunk,
            )
            target_output_frames = num_frames if not use_interpolate_prompt else None
        else:
        ...
        # Whether the short-tier indices include a prefix index (controlled by transformer config).
        ...
        if keep_first_frame:
            ...
            if _use_raw_sink_frames:
                # Raw sink mode: prefix index handled via sink_latents rope; short tier uses 1x segment only.
        ...
        for k in range(num_latent_chunk):
            ...
            if keep_first_frame:
                ...
                if _use_raw_sink_frames:
                    # Raw sink mode: x0 becomes the first sink token instead of prepending to short tier.
                ...
                if _use_raw_sink_frames:
                    # No x0 source when keep_first_frame=False; nearby sink still takes the last short-tier frame.
                    nearby_sink_latents = latents_history_short[:, :, -1:, :, :].clone()
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                ...
                if vae_decode_type == "long":
                    pass  # Decode full sequence after the loop.
                elif vae_decode_type == "persistent":
                    # Decode chunk while reusing VAE feat-map cache; bit-identical to long mode.
                    new_pixels = self._decode_chunk_persistent_cache(
                        z_chunk=real_history_latents[:, :, -num_latent_frames_per_chunk:],
                        is_first_chunk=(k == 0),
                        latents_mean=latents_mean,
                        latents_std=latents_std,
                        vae_dtype=vae_dtype,
                    ...
                    if target_output_frames is not None:
                        current_frames = 0 if history_video is None else history_video.shape[2]
                        remaining_frames = max(target_output_frames - current_frames, 0)
                        new_pixels = new_pixels[:, :, :remaining_frames]
                    ...
                    if on_chunk_decoded is not None:
                        on_chunk_decoded(k, new_pixels)
                elif vae_decode_type == "streaming":
                    # Decode chunk with overlap warm-up latents prepended; discard warm-up pixels afterwards.
                    if k == 0:
                        decode_latents = real_history_latents[:, :, -num_latent_frames_per_chunk:]
                        discard_pix = 0
                    else:
                        # Prepend overlap latents for VAE causal warm-up; compute pixel discard offset.
                        overlap = min(streaming_overlap_latent, real_history_latents.shape[2] - num_latent_frames_per_chunk)
                        decode_latents = real_history_latents[:, :, -(num_latent_frames_per_chunk + overlap):]
                        discard_pix = max(
                            0,
                            overlap * self.vae_scale_factor_temporal - (self.vae_scale_factor_temporal - 1),
                        )

                    decode_latents = decode_latents.to(vae_dtype) / latents_std + latents_mean
                    decoded_pixels = self.vae.decode(decode_latents, return_dict=False)[0]
                    new_pixels = decoded_pixels[:, :, discard_pix:, :, :] if discard_pix > 0 else decoded_pixels
                    if target_output_frames is not None:
                        current_frames = 0 if history_video is None else history_video.shape[2]
                        remaining_frames = max(target_output_frames - current_frames, 0)
                        new_pixels = new_pixels[:, :, :remaining_frames]
                    ...
                    # "default": per-chunk independent decode; minimal VRAM but boundary artifacts.
                    current_latents = (
                    ...
                    if on_chunk_decoded is not None:
                        on_chunk_decoded(k, current_video)

        self._current_timestep = None

        # Clear persistent VAE cache after the chunk loop to prevent leakage.
        if vae_decode_type == "persistent":
            self.vae.clear_cache()

        if output_type != "latent":
            if vae_decode_type == "long":
                # Decode the full accumulated latent sequence at once; no boundary artifacts but high VRAM.
            ...
            if target_output_frames is not None:
                generated_frames = min(history_video.size(2), target_output_frames)
            else:
